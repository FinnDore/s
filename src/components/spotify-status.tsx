...
export const UnwrappedSpotifyStatus = () => {
    const { data } = useQuery({
        queryKey: ['spot'],
        queryFn: async () => ({
            currentSong: await fetch('http://localhost:3001').then(
    ...
    return (
        <div className="spotify-status flex w-[100vw] absolute z-20 bottom-0 rounded-md p-2 py-3 text-white sm:p-4">
            <a
                rel="noreferrer"
                target="_blank"
...
const ProgressBar = memo(function ProgressBar({
    ...
    return (
        <div className="h-[.2rem] w-full rounded-full ">
            <div
                className="m-w-full h-full rounded-full bg-white  transition-all"
                style={{
